...
function ReactTable({ columns, data, setCourseData }) {
  ...
  const handleConfirmDelete = async () => {
    ...
    if (id) {
      try {
        ...
        // navigate("/");
        // } else {
        await axios.delete(`${server}/api/course_module/${id}`, {
          // headers: {
          //   Authorization: `Bearer ${token}`
          // }
        });
        const updatedcourses = data.filter((course) => course._id !== id);
        ...
        <Stack direction="row" spacing={4} justifyContent="space-between" alignItems="center" sx={{ p: 3, pb: 0 }}>
          ...
          <Stack direction="row" alignItems="center" spacing={3} textAlign="justify">
            ...
            <Button
              ...
              startIcon={<Add />}
              onClick={() => navigate('/apps/create-course')} // Single line redirect
            >
              Add New Courses
            </Button>
          </Stack>
              ...
              .map((row) => (
                ...
                <TableRow key={row._id}>
                  ...
                  <TableCell style={{ width: '60%' }}>
                    <Stack direction="row" spacing={1.5} alignItems="center">
                      <Typography>{row?.courseOverview?.split(' ').slice(0, 20).join(' ')}...</Typography>
                    </Stack>
                  </TableCell>
                    ...
                    >
                      <IconButton
                        ...
                        onClick={(e) => {
                          ...
                          handleClose();
                          // handleEdit(row);
                          navigate(`/apps/edit-course/${row._id}`);
                        }}
                      >
                        <Edit />
                      </IconButton>
...
const CoursesListPage = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      ...
      // else {
        try {
          const response = await axios.get(`${server}/api/course_module`, {
            // headers: {
            //   Authorization: `Bearer ${token}`
            // }
          });
          ...
          if (response.data) {
            ...
            // console.log('api data', response.data.courses);
          } else {
  ...
  const columns = useMemo(
    () => [
      ...
      { Header: 'Course Overview', accessor: 'courseoverview' },
  ...
  return (
    <MainCard content={false}>
      <ScrollX>
        <ReactTable columns={columns} data={CourseData} setCourseData={setCourseData} />
        {/* <SearchBar filterText={filterText} setFilterText={setFilterText} handleSearchChange={handleSearchChange} /> */}
      </ScrollX>
    </MainCard>
  );
