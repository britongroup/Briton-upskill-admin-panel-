...
function ReactTable({ columns, data, setContactData }) {
  ...
  const handleEdit = (details) => {
    ...
    console.log('details email', details);
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      const response = await axios.put(`${server}/api/contacts/${editModal.details._id}`, requestData, {
        // headers: {
        //   Accept: '*/*',
      ...
      if (response.status === 200) {
        ...
        try {
          // const token = localStorage.getItem('token');
          const updatedData = await axios.get(`${server}/api/contacts`, {
            headers: {
              Accept: '*/*',
              // Authorization: `Bearer ${token}`
            }
          });
          setContactData(updatedData.data);
        ...
        toast.error('Details Not updated.. Try Again');
      }
    } catch (error) {
  ...
  const handleSaveNewContact = async () => {
    try {
      ...
      // const token = localStorage.getItem('token');
      // console.log(token);
      const response = await axios.post(`${server}/api/contacts/create`, requestData, {
        headers: {
          Accept: '*/*',
          // Authorization: `Bearer ${token}`
        }
...
console.log('updated contact',updatedData?.data)
          setContactData(updatedData?.data);
  ...
  const handleConfirmDelete = async () => {
    ...
    if (id) {
      try {
        ...
        console.log('updatred custoier-->',updatedCustomers?.data)

        setContactData(updatedCustomers);
      ...
      {addNewModal.open && (
        <div className="fixed top-0 left-0 w-[100vw] h-full flex items-center overflow-y-auto justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-md max-w-md w-full mx-4 max-h-[70vh] overflow-y-auto">
            ...
            >
              <h3 className="text-2xl font-medium text-gray-800 mb-4">Enter New Contact Details</h3>
              ...
              <div>
                <label htmlFor="email" className="text-sm font-medium text-gray-800 light:text-gray-300 block mb-2">
                  Email
                </label>
                <input
                  type="email"
              ...
              <div>
                <label htmlFor="mobileNumber" className="text-sm font-medium text-gray-800 light:text-gray-300 block mb-2">
                  Mobile Number
                </label>
                <input
                  type="number"
              ...
              <div>
                <label htmlFor="message" className=" text-sm font-medium text-gray-800 light:text-gray-300 block mb-2">
                  Message
                </label>
        ...
        <div className="fixed top-0 left-0 w-[100vw] h-full flex items-center justify-center align-middle  bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-md max-w-md w-full mx-4 max-h-[70vh] overflow-y-auto">
            <div className="flex justify-between p-2">
              <div className="div">
                <h3 className="text-2xl font-medium text-gray-800 mb-4">Edit Contact Details</h3>
              </div>
            ...
            >
              ...
              <div>
                ...
                {console.log('editemail', editemail, editname)}
              </div>
...
const ContactListPage = () => {
  ...
  return (
    <MainCard content={false}>
      <ScrollX>
        <ReactTable columns={columns} data={ContactData} setContactData={setContactData} />
        {/* <SearchBar filterText={filterText} setFilterText={setFilterText} handleSearchChange={handleSearchChange} /> */}
      </ScrollX>
    </MainCard>
  );
