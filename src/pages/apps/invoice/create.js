...
// Resetting fields
  ...
  const handlerCreate = (values, { resetForm }) => {
    ...
    const invoiceDetail = values.invoice_detail.map((item) => ({
      name: item.name,
    ...
    const requestBody = {
      ...
      to: {
        ...
        city: values.customerInfo.city,
        number: values.customerInfo.phone,
    ...
    // Send the requestBody to your API
    // You can use fetch or axios to make the API request
    const token = localStorage.getItem('token')
    console.log('RequestBody---->',requestBody)
    fetch( `${process.env.REACT_APP_API_URL}/api/invoice/create-invoices`, {
      method: 'POST',
      headers: {
      ...
      .then((data) => {
        ...
        if(data.success === true){
          ...
          toast('Invoice send Successfully');
          console.log('data successfully send----->',data)
          resetForm()
        }
        else{
    ...
    <MainCard>
      <Formik
        initialValues={{
          ...
          due_date: null,
          cashierInfo: {
            name: 'Doledge India OPC Pvt Ltd',
            address: 'Noida,Uttar Pradesh',
            phone: '9090909090',
            email: 'doledge@gmail.com'
          },
          customerInfo: {
            address: '',
            email: '',
        ...
        onSubmit={(values, { resetForm }) => {
          handlerCreate(values,resetForm);
        }}
      >
        {({ handleBlur, errors, handleChange, handleSubmit,resetForm, values, isValid, setFieldValue, touched }) => {
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6} md={3}>
                  <Stack spacing={1}>
                    ...
                    <FormControl sx={{ width: '100%' }}>
                      ...
                      >
                        {/* <MenuItem value="unpaid" name='unpaid'> */}
                        <MenuItem value="unpaid" name='unPaid'>
                            Unpaid
                            </MenuItem>;
                     
                        {/* </MenuItem> */} 
                        {/* <MenuItem value="paid" name='paid'>Paid</MenuItem> */}
                        {/* <MenuItem value="Unpaid" name='unpaid'>Unpaid</MenuItem> */}
                        {/* <MenuItem value="Cancelled" name='cancelled'>Cancelled</MenuItem> */}
                      </Select>
                    </FormControl>
                  </Stack>
                ...
                <Grid item xs={12} sm={6} md={3}>
                  <Stack spacing={1}>
                    ...
                    <FormControl sx={{ width: '100%' }} name="date" error={Boolean(touched.date && errors.date)}>
...
{/* Due Date */}
                ...
                <Grid item xs={12} sm={6} md={3}>
                  <Stack spacing={1}>
                    ...
                    <FormControl sx={{ width: '100%' }} name="dueDate" error={Boolean(touched.due_date && errors.due_date)}>
                      <LocalizationProvider dateAdapter={AdapterDateFns}>
                        <DatePicker
                          format="dd/MM/yyyy"
                      ...
                      <Grid item xs={12} sm={8}>
                        <Stack spacing={2}>
                          ...
                          <Stack sx={{ width: '100%' }}>
                            ...
                            <Typography color="secondary">{values?.cashierInfo?.city}</Typography>
                      ...
                      <Grid item xs={12} sm={8}>
                        <Stack spacing={2}>
                          ...
                          <Stack sx={{ width: '100%' }}>
                            ...
                            <Typography variant="subtitle1" className='text-left' style={{width:'100%'}}>{values?.customerInfo?.name}</Typography>
                            </div>

                            <div className='flex'>
                            <Typography  style={{width:'100px'}}>Address {' '}</Typography>
                            <Typography color="secondary" className='text-left' style={{width:'100%'}}>{values?.customerInfo?.address}</Typography>

                            </div>

                            <div className='flex'>
                            <Typography style={{width:'100px'}}>City {' '}</Typography>
                            <Typography color="secondary" className='text-left' style={{width:'100%'}}>{values?.customerInfo?.city}</Typography>
                            </div>
                            {/* {console.log('{values?.cashierInfo?.city--->',values?.customerInfo?.city)} */}
                              {/* <Typography color="secondary">asadad sda d a</Typography> */}

                              <div className='flex'>
                              <Typography  style={{width:'100px'}}>Phone {' '}</Typography>
                            <Typography color="secondary" className='text-left' style={{width:'100%'}}>{values?.customerInfo?.phone}</Typography>
                            </div>
                            <div className='flex'>
                            <Typography  style={{width:'100px'}}>Email {' '}</Typography>
                            <Typography color="secondary" className='text-left' style={{width:'100%'}}>{values?.customerInfo?.email}</Typography>
                            </div>
                          </Stack>
                        </Stack>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <Box textAlign="right" color="secondary.200">
                          ...
                          <AddressModal
                            ...
                            handlerAddress={(customerInfo) => setFieldValue('customerInfo', customerInfo)}
                          />
                        </Box>
                      </Grid>
                    </Grid>
                                ...
                                {values.invoice_detail?.map((item, index) => (
                                  <TableRow key={item.id}>
                                    ...
                                    <InvoiceItem
                                      ...
                                      Blur={handleBlur}
                                      handleChange={handleChange} // Add this line
                                      errors={errors}
                                      touched={touched}
                                    />
                                  </TableRow>
                ...
                <Grid item xs={12} sm={12}>
                  <Stack direction="row" justifyContent="flex-end" alignItems="flex-end" spacing={2} sx={{ height: '100%' }}>
                    ...
                    <Button type='submit' color="primary" variant="contained" className="bg-blue-500 text-white hover:bg-blue-600">
                      Create & Send
                    </Button>
                    <InvoiceModal
                      isOpen={isOpen}
