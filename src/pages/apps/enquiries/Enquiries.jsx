...
function ReactTable({ columns, data, setEnquiryData }) {
  ...
  const handleConfirmDelete = async () => {
    ...
    if (id) {
      try {
        ...
        // navigate("/");
        // } else {
        await axios.delete(`${server}/api/registermodal/${id}`, {
          // headers: {
        ...
        setEnquiryData(updatedCustomers);
            ...
            {paginatedData
              ...
              .map((row) => (
                ...
                <TableRow key={row._id}>
                  ...
                  <TableCell className="" style={{ width: '45%' }}>
                    <Stack direction="row" spacing={1.5} alignItems="center">
...
const EnquiriesListPage = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      //  else {
        try {
          const response = await axios.get(`${server}/api/registermodal`, {
            // headers: {
            //   Authorization: `Bearer ${token}`
            // }
          });
  ...
  return (
    <MainCard content={false}>
      <ScrollX>
        <ReactTable columns={columns} data={EnquiryData} setEnquiryData={setEnquiryData} />
        {/* <SearchBar filterText={filterText} setFilterText={setFilterText} handleSearchChange={handleSearchChange} /> */}
      </ScrollX>
    </MainCard>
  );
